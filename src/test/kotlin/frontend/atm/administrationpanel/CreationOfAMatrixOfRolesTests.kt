...
class CreationOfAMatrixOfRolesTests : BaseTest()  {
    ...
    @Test
    @Issue("ATMCH-5774")
    @DisplayName("The user is an Employee of a company with a switched-on “ETC company” feature in Admin Panel." +
    ...
    fun employeeUserSwitchedToETCTokensPage() {
        ...
        with(openPage<AtmProfilePage>(driver) { submit(etcEmployee) }) {
            assertThat("Unexpected participant role", getParticipantRole(),
    ...
    @TmsLink("ATMCH-5374")
    @Test
    @DisplayName("The user is an Employee of a company with a switched-on “Industrial” feature in Admin Panel." +
    ...
    fun employeeUserSwitchedOnIndustrialFeature() {
        ...
        with(openPage<AtmMarketplacePage>(driver)) {
            ...
            val expectedTokens = listOf("INDUSTRIAL TOKEN", "VALIDATION TOKEN", "FRACTIONAL TOKEN", "CURRENCY TOKEN")
                .sorted().toString()
            assertThat("One or more tokens type are incorrect", tokens, equalTo(expectedTokens)
            )
